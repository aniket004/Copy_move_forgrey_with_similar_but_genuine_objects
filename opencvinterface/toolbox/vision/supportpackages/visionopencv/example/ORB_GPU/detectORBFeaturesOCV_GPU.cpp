...
// manner. 
///////////////////////////////////////////////////////////////////////////////
mxArray * getORBKeypointLocations(cv::gpu::GpuMat & keypoints)
{
    int numKeypoints = keypoints.cols;

    // Create the 2D mxGPUArray for the output locations
    mwSize dims[] = {numKeypoints, 2};
    ...
    cv::gpu::GpuMat outputLocations(2, numKeypoints, CV_32FC1, locations);
    ...
    cv::Range rowRange(cv::gpu::ORB_GPU::X_ROW,
            cv::gpu::ORB_GPU::RESPONSE_ROW);
    
    cv::Range colRange = cv::Range::all();

    cv::gpu::GpuMat gpuLocations(keypoints, rowRange, colRange); 
    
    // Add 1 to the location for 1-based indexing.
    cv::gpu::add(gpuLocations, 1.0F, outputLocations);
...
{  
    ...
    Ptr<gpu::GpuMat> gpuImage = ocvMxGpuArrayToGpuMat_uint8(prhs[0]);
        
    // Set up the detector (using default algorithm parameters)
    gpu::ORB_GPU featureDetector;
    
    gpu::GpuMat keypoints;  // detector output

    // Run the ORB detector on the GPU
    featureDetector(*gpuImage, gpu::GpuMat(), keypoints);
