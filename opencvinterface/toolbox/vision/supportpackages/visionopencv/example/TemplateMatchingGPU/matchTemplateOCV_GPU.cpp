...
{  
    ...
    cv::Ptr<cv::gpu::GpuMat> templateImgCV = ocvMxGpuArrayToGpuMat_uint8(prhs[0]);
    cv::Ptr<cv::gpu::GpuMat> imgCV         = ocvMxGpuArrayToGpuMat_uint8(prhs[1]);

    // Allocate output matrix
    int outRows = imgCV->rows - templateImgCV->rows + 1;
    int outCols = imgCV->cols - templateImgCV->cols + 1;

    cv::gpu::GpuMat outCV((int)outRows, (int)outCols, CV_32FC1);

    // Run the OpenCV template matching routine
    cv::gpu::matchTemplate(*imgCV, *templateImgCV, outCV, CV_TM_CCOEFF_NORMED );
